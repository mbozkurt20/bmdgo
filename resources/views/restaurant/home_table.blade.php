                        ...
                        <div class="table-responsive">
                            <table class="order-table shadow-hover card-table table table-bordered table-hover shadow-hover card-table text-black" style="min-width: 845px">
                                ...
                                @foreach($tumu->where('status', $statusKey) as $order)
                                    @include('restaurant.partials.orders.orders_row', ['order' => $order])
                                @endforeach
                                </tbody>
                            </table>
                        </div>
...
<script>
    function printOrder(orderId) {
        fetch('/restaurant/printed/' + orderId) // veya route() ile endpoint
...
<script>
    ...
    function generateOrderRowHtml(order) {
        ...
        if (platform.toLowerCase() === 'yemeksepeti') {
            ...
            platformHtml = `<a style="padding: 5px" class="btn btn-primary btn-rounded">
            ${restaurantName} /
            <img src="{{ asset('theme/images/trendyolyemek.png') }}" style="height: 15px">
        </a>`;
        } else if (platform.toLowerCase() === 'migros') {
            platformHtml = `<a style="padding: 5px;background: #000080;border-color: #6244be; font-size: 14px" class="btn btn-primary btn-rounded">
            ${restaurantName} /
